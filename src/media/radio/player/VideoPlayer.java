...
class VideoFilter implements FilenameFilter {
	public boolean accept(File dir, String name) {
		return (name.endsWith(".3gp")||name.endsWith(".mp4")||name.endsWith(".webm")||name.endsWith(".mkv"));
	}
}
...
public class VideoPlayer extends Activity implements SurfaceHolder.Callback, SensorListener{
	...
	/** Called when the activity is first created. */
	 ...
	 public void onCreate(Bundle savedInstanceState) {
	    ...
	    btnPrevVideo = (ImageButton)findViewById(R.id.btnPrevVideo);
	    btnNextVideo = (ImageButton)findViewById(R.id.btnNextVideo);
	 ...
	 public void onClickFullVideo(View v){
		 Intent intent = new Intent(VideoPlayer.this, ViewVideo.class);
         intent.putExtra("video", getVideo);
         startActivity(intent);
	 }   
	 
	 public void onClickNextVideo(View v){
		 Animation shake = AnimationUtils.loadAnimation(this, R.anim.shake);
		 	findViewById(R.id.btnNextVideo).startAnimation(shake);
		 
		 nextVideo();
	 }
	 
	 public void onClickPrevVideo(View v){
		 Animation shake = AnimationUtils.loadAnimation(this, R.anim.shake);
		 	findViewById(R.id.btnPrevVideo).startAnimation(shake);
	 ...
	 public void updateVideoList() {
			...
			File fileList[] = home.listFiles(new VideoFilter());
			if (fileList != null) {
				for (File file : fileList ) {
					video.add(file.getName());
				}
			}
		}
	 
	 private String playVideo(String file) {
			mediaPlayer.reset();
	 ...
	 private void nextVideo() {
		 	updateVideoList();
		 	if (++currentPosition >= video.size()) {
				currentPosition = 0;
			} else {
				playVideo(getPath + video.get(currentPosition));
				seekBar();
			    startPlayProgressUpdater();
			     
			    btnPlayVideo.setImageResource(R.drawable.pause); 
			    stateMediaPlayer = stateMP_Playing;
				Toast.makeText(this, getPath + video.get(currentPosition), Toast.LENGTH_SHORT).show();
			}
